...
let personLarge = {
  ...
  address: address, //створюємо об'єкт address
};
...
function copyObject(obj) {
  var copiedObj = { ...obj };  // Використовуємо синтаксис деструктурізації {...person} для створення нового об'єкта з тими ж властивостями
  return copiedObj; // Повертаємо новий об'єкт
}

var copiedAnimal = copyObject(animal);
console.log(copiedAnimal);
...
function hasProperty(obj, property) {
  ...
  } else { // інашке повертаємо Property ${property} does not exist.
...
function changeRole(array, newRole) { 
  for (let user of array) {  // Ітеруємося по масиву об'єктів за допомогою циклу "for of"
    user.role = newRole;   // Змінюємо роль кожного користувача на нове ім'я
  } 
...
function printProductDetails(obj) {
  ...
  console.log(`Manufacturer: ${companyName} (${country})`);  // Виводимо productName, price, companyName та country на консоль
}
...
planet2 = planet1; // Присвоємо obj2 значення об'єкту obj1

console.log();  // Виводимо результат порівняння об'єктів
...
addProperty(newArr);  // Створимо масив newArr з новою властивістю за допомогої нашої функції в яку передамо [1, 2, 3, 4, 5]
